...
public class FlipCards {
  ...
  public void reloadTexture(int frontIndex, View frontView, int backIndex, View backView) {
    synchronized (this) {
      ...
      if (Logger.ENABLE_DEBUG) {
        Logger.d("reloadTexture: activeIndex %d, front %d, back %d, angle %.1f",
  ...
  public synchronized boolean handleTouchEvent(MotionEvent event, boolean isOnTouchEvent) {
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (state == STATE_TOUCH && event.getY() >6.0*controller.getContentHeight()/10.0) {
  ...
  private void setState(int state) {
    if (this.state != state) {                        /*
			if (Logger.ENABLE_DEBUG)
				Logger.i("setState: from %d, to %d; angle %.1f", this.state, state, angle);
			*/
      this.state = state;
      animatedFrame = 0;
    }

...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  private Handler handler = new Handler(new Handler.Callback() {
    ...
    public boolean handleMessage(Message msg) {
      if (msg.what == MSG_SURFACE_CREATED) {
        ...
        Logger.w("Unknown msg.what: " + msg.what);
      }
      return false;
    }
  });
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    if (Logger.ENABLE_DEBUG) {
      Logger.d("onLayout: %d, %d, %d, %d; child %d", l, t, r, b, bufferedViews.size());
    }
  ...
  private void debugBufferedViews() {
    if (Logger.ENABLE_DEBUG) {
      Logger.d("bufferedViews: %s; buffer index %d, adapter index %d", bufferedViews, bufferIndex,
              adapterIndex);
    }
  }
  ...
  void flippedToView(final int indexInAdapter, boolean isPost) {
    if (Logger.ENABLE_DEBUG) {
      Logger.d("flippedToView: %d, isPost %s", indexInAdapter, isPost);
    }
    ...
    if (indexInAdapter >= 0 && indexInAdapter < adapterDataCount) {
      ...
      if (indexInAdapter == adapterIndex + 1) { //forward one page
        ...
        Logger.e("Should not happen: indexInAdapter %d, adapterIndex %d", indexInAdapter,
                adapterIndex);
      }
    } else {
